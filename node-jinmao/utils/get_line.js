// 本脚本接受编号后的md文本内容，将文本内容与提示词拼接后，通过 llmClient.chat() 发送给 DeepSeek 小模型进行行号识别，
// 最后输出识别出来的 startline 和 endline。
// 编写脚本时应首先进行输入校验，确保不是空字符串、不是null、不是undefined、不会出现注入攻击。
// 应为错误信息进行编号返回，如200等
// 返回值为对象（非 JSON 字符串），包含 startline 和 endline。同时错误信息也应包含在返回值，方便调用时判断是否成功。
...
// 注意：DeepSeek API 调用已统一迁移至 llm_client.js，由它统一管理客户端、看门狗、心跳和计费
const llmClient = require("./llm_client");            // 统一 LLM 客户端（封装 DeepSeek API 调用 + 自动计费）
const promptConfig = require("../config/prompt.json"); // 提示词路径配置
const fs = require("fs");                              // 文件系统：读取提示词模板文件
const path = require("path");                          // 路径处理：解析提示词模板文件路径
const { validateString } = require("./input_validator"); // 输入验证：防止空值/注入攻击
 ...
 *   1. 输入验证（空值/类型/注入/长度）
 *   2. 将提示词模板与编号后的文本拼接
 *   3. 通过 llmClient.chat() 调用 DeepSeek API（小模型 + 关闭思考模式）
 ...
 *
 * @param {string} userId - 用户 ID（用于计费关联，传递给 llmClient）
 ...
 */
async function getLine(userId, indexedMarkdown) {
    ...
    console.log("[get_line] 提示词与文本拼接完成，总长度：" + fullPrompt.length + " 字符。准备通过 llmClient 调用 DeepSeek API（小模型，思考模式已关闭）。");

    // ========== 步骤 3：通过 llmClient 调用 DeepSeek API（小模型 + 关闭思考模式） ==========
    // llmClient.chat() 内部已统一处理：客户端初始化、看门狗、心跳、计费
    const chatResult = await llmClient.chat(userId, "getline", {
        modelSize: "small",                                      // 使用小模型（deepseek-v4-flash）
        messages: [{ role: "system", content: fullPrompt }],     // 系统提示词（含待分析文本）
        thinking: { type: "disabled" },                          // 关闭思考模式（任务简单，无需深度推理）
        stream: false,                                            // 非流式输出，等待完整结果
    });

    // ========== 步骤 4：校验 llmClient 返回状态 ==========
    if (chatResult.code !== 200) {
        // llmClient 调用失败（网络超时、鉴权失败、服务端 5xx 等）
        console.error("[get_line] llmClient 调用失败（code=" + chatResult.code + "）：" + (chatResult.message || "未知错误"));
        return {
            code: 500,
