<!--
============================================================================
文件名：pages/mobile-quiz/report.vue（手机端刷题报告页面）
...
<template>
  <div class="min-h-screen flex flex-col transition-colors duration-500"
    ...
    <!-- 顶部栏 -->
    <header class="flex items-center px-4 py-3 border-b transition-colors duration-500"
    ...
    <!-- 报告内容 -->
    <div v-else-if="report" class="flex-1 overflow-y-auto px-4 py-5">
      ...
      <div v-if="expandedIdx >= 0" class="rounded-[10px] border p-4 mb-4 transition-all duration-500"
        ...
        <div class="flex items-center gap-2 mb-3">
          ...
          <span v-else class="text-gray-400 dark:text-gray-500 text-sm">批改中</span>
        </div>

        <!-- 题干 -->
        <p class="text-sm text-black dark:text-white mb-3 leading-relaxed transition-colors duration-500">
          {{ currentItem?.questionContent }}
        </p>

        <!-- 用户答案 -->
        <div class="text-xs mb-2">
          <span class="text-gray-400 dark:text-gray-500">你的答案：</span>
          <span class="text-black dark:text-white">{{ currentItem?.userAnswer || '未作答' }}</span>
        </div>

        <!-- 参考答案 -->
        <div class="text-xs mb-3">
          <span class="text-gray-400 dark:text-gray-500">参考答案：</span>
