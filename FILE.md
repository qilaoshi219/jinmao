...
| `billing.js` | 账单查询路由：GET /api/v1/billing 返回用户 VIP 等级、余额、扣费记录分页列表（需 Token） | **新建 2026-07-29** |
...
| `quiz_service.js` | 刷题核心业务：会话管理、题目抽样、判题、进度保存、交卷，新增每日活动记录（非阻塞） | **修改 2026-07-29** |
...
| `src/pages/home/index.vue` | 首页模板 — NERV 蓝色战术风格，4 个统计卡片改为数据绑定（学习时长/已完成章节/正确率/连续天数） | **修改 2026-07-29** |
| `src/pages/home/script.js` | 首页业务逻辑，新增 stats 状态、loadStats()、formatDuration() | **修改 2026-07-29** |
