...
| `handover.ps1` | **代码移交打包脚本**（完整源码+Git历史+开发文档 → jinmao-handover-*.zip，供继承开发者接手；在临时克隆中清理历史里的敏感凭据与冗余大文件后打包，本地仓库不受影响；可选 -IncludeTests / -IncludeMedia） | **新建 2026-08-04** |
| `部署说明.md` | **三环境部署说明**（Windows / Linux / 宝塔：环境准备、.env 配置、数据库初始化、启动、反代、验证、FAQ、检查清单） | **新建 2026-08-04** |
| `ftp_upload.ps1` | FTP 上传脚本（仅负责上传文件，数据库迁移/前端构建手动执行，后端重启在宝塔面板操作；`.env` 排除、`.env.example` 模板随包上传） | 2026-08-01 |
| `ftp_upload_prod.ps1` | **生产环境 FTP 上传脚本**（上传到 `/www/wwwroot/jinmao-prod`，需生产专用 FTP 账户[主目录 jinmao-prod]，红色警告 + yes 确认；仅大版本上线推送使用，日常测试用 ftp_upload.ps1） | **修改 2026-08-01** |
...
| `双环境部署指南.md` | **测试/生产双环境部署指南**（架构总览、生产首次部署、日常发布流程、回滚、常见问题） | **新建 2026-08-01** |
| `待办.md` | 待办事项 | — |
| `测试命令.md` | 测试命令参考 | — |
| `测试报告.md` | 测试结果报告 | — |
